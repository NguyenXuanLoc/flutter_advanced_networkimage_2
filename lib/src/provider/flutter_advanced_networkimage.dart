...
class AdvancedNetworkImage extends ImageProvider<AdvancedNetworkImage> {
  ...
  Future<ui.Codec> _loadAsync(
    ...
    if (useDiskCache) {
      try {
        ...
        if (_diskCache != null) {
          ...
          return decode(await ImmutableBuffer.fromUint8List(_diskCache),
              getTargetSize: (s, a) =>
                  TargetImageSize(height: key.height, width: key.width));
        }
      } catch (e) {
    ...
    if (key.fallbackImage != null)
      return decode(await ImmutableBuffer.fromUint8List(key.fallbackImage!),
          getTargetSize: (s, a) =>
              TargetImageSize(height: key.height, width: key.width));
